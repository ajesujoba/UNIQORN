...
def main(argv):
    ...
    for qid in list(questionids.keys()):
        ...
        f2 = open(outputdir+'/tripscore_'+qid+'.txt','w')
        ccnt=0
        for keys in kg:
            ...
            for tiples in texts:
                ...
                f1.write('triple_' + str(ccnt) + ' ##### ' +  str(score) + ' #### ' + tiples2)
                f1.write('\n')
                if tiples[1].strip().lower()!='instance of' and tiples[1].strip().lower()!='occupation':
                    f2.write('triple_'+str(ccnt) + '|' + 'triple_' + str(ccnt) + '|'+ str(score))
                    f2.write('\n')
                ccnt = ccnt + 1
        f1.close()
