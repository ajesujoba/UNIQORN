...
def formatTriple(triples):
    ...
    for line in allfacts:
        ...
        if triple[0]!='Qualifier':
            ...
            #n1_id=triple[0]
            #print(triple)
            n1=replacespace(triple[0].strip().strip('"').replace(':',' ')).strip()
            try:
                score_n1=float(triple[1].strip())
                score_n3=float(triple[3].strip())
            except ValueError:
                continue
            n2=replacespace(triple[2].strip().strip('"').replace(':',' ')).strip()
            n3=replacespace(triple[4].strip().strip('"').replace(':',' ')).strip()
            ...
            try:
                ...
                continue
            matched_qual=''
            qual1=replacespace(triple[2].strip().strip('"').replace(':',' ')).strip()
            #qual2_id=triple[3].decode('utf-8')
            qual2=replacespace(triple[3].strip().strip('"').replace(':',' ')).strip()
            try:
...
def call_main_GRAPH_KG(spofile,topl,typefile, entity_dir, questn_tokens, path_file, pred_aliases_dir, config,topcontext=0,stempred=False,degeneratex=False,pred_count={}):
    try:
        ...
        entity_names = [replacespace(prepro(item)).strip() for item in entity_names]
...
def call_main_GRAPH_TEXT(G,topl,textspofile,textcooccur,textcontextscores,hearst_file,questn_tokens,topcontext,config,pred_count={} ):
    ...
    if Add_Type_flag == 1:
        ...
        G = add_type_edges(G,final_hearst,topl)
...
def add_type_edges(G,HP,topsnipets):
    ...
    for pat in HP:
        ...
        if snipet not in topsnipets:
            ...
            #print('hearst not in snippet', snipet, topsnipets)
            continue
        #print('Found an hearst')
        score = pat[4]
        ...
        if p1 in G.nodes():
            ...
            if p2 not in G.nodes():#type node does not exist
                #print('Adding types nodes |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||')
                G=add_node_triple_TEXT(G, p2, [], [], []) #Adding a "Type" node; not using typical nodes with only labeled "type"
                G.add_edge(p1, p2, weight=score, wlist=[score], etype='Type',did=[pat[2]],dtitle=[],sid=[pat[3]],source='TEXT')
            else:
                if (p1,p2) not in G.edges():
                    G.add_edge(p1, p2, weight=score, wlist=[score], etype='Type',did=[pat[2]],dtitle=[],sid=[pat[3]],source='TEXT') #Assuming there do not exist other edges between entity and "Type" nodes
                    ...
                    sid = data['sid']
                    sid.append(pat[3])
                    G.add_edge(p1, p2, weight=score, wlist=wlit, etype='Type',did=did,dtitle=[],sid=sid,source='TEXT') #Assuming there do not exist other edges between entity and "Type" nodes
...
def getSPO(f22,f33):
    ...
    for line in f22:
        ...
        doc_title=triple[1]
        sent_id=triple[2]
        n1=replacespace(triple[3]).lower()
        d1=float(triple[4])
        n2=replacespace(triple[5]).lower()
        d2=float(triple[6])
        n3=replacespace(triple[7]).lower()
        if (n1,n2,n3) not in unique_SPO_dict:
            unique_SPO_dict[(n1,n2,n3)]={}
            unique_SPO_dict[(n1,n2,n3)]['d1']=[]
            unique_SPO_dict[(n1,n2,n3)]['d2']=[]
...
def add_entity_alignment_edges1(G,mention_dict,g_ent):
    xdict = defaultdict()
    for n1 in G.nodes():
        nn1=n1.split(':')
        if nn1[1]=='Entity':
...
def add_predicate_alignment_edges1(G,g_pred,gdict):
    xdict = defaultdict()
    if MAX_MATCH==0:
        for n1 in G.nodes():
            nn1=n1.split(':')
...
def add_type_alignment_edges1(G,g_type,gdict):
    xdict = defaultdict()
    if MAX_MATCH==0:
        for n1 in G.nodes():
            nn1=n1.split(':')
...
def call_main_GRAPH(qid,kgspofile,kgtypefile,kgcontextscores,kgentity_dir, kgquestn_tokens,path_file,kgpred_aliases_dir, textspofile,textcooccur,textcontextscores, qtfile, hearst_file, config,topk,gdict,finalxg='', finalcorner='',et=0,pt=0,tt=0,ablate=0,we=''):
    ...
    try:
        txtsn = [splittext(line.strip()) for line in open(textcontextscores,'r')]
    except FileNotFoundError as err:
        txtsn = []

    topl = []
    if len(kgsn) > 0:
        topl.extend(rank_and_select_triples(list(set(kgsn)),topcontext=topk))

    if len(txtsn) > 0:
        topl.extend(rank_and_select_triples(list(set(txtsn)),topcontext=topk))
    ...
    #call the TEXT
    G, questn_tokens, txttrip = call_main_GRAPH_TEXT(G,topl,textspofile,textcooccur,textcontextscores,hearst_file,kgquestn_tokens,topk,config,pred_count=pred_count )
    corner_TEXT= getcornerstone_TEXT(G,kgquestn_tokens)
    print(len(G.nodes()))
    #print(len(G.edges()))
    #print(corner_TEXT)
