...
def get_cost(t,G):
    ...
    for (n1,n2) in t.edges():
        ...
        #if times > 0 and currenttime-starttime > 5*times:
        #break
        if (n1,n2) in G.edges():
...
def save_GST(GST_set,v,g1,corner,G,starttime,times):
    g=nx.Graph()
    edge_added=0
...
def get_GST(Q,T,P,G,no_GST, corner,verbose,times):
    pop_hist={}
    ite=0
    pop_cov=-1
    merge_cov=-1
    ...
    while len(Q)>0:
        ...
        if p==P:
            ...
            GST_set,New_GST_flag,pot_ans_flag=save_GST(GST_set,v,T[v][p],corner,G,starttime,times)
            if New_GST_flag==1 and pot_ans_flag==1:
                GST_count+=1
            if GST_count==no_GST and final_GST_flag==0:
                final_GST_cost=get_cost(T[v][p],G)
                final_GST_flag=1
        currenttime = time.time() # 30.11
        if times > 0 and currenttime-starttime > 5*times:
            break
        ...
        #break the whole loop if u in G.neighbors(v) is not completed on Soumajit recommendation 30.11.
        if breakwhile == 1:
            break
        breakwhile2 = 0 #30.11
        ...
        if breakwhile2 == 1:#if you break from p2 in all_p2 break the entire while
            break
...
def get_rank_docs(rank_doc):
	...
	for tup in rank_doc:
		...
		if rank>=1 and rank<=10:
			...
			if rank>=11 and rank<=20:
				doc_11_20.append(doc)
				if aa==1:
					ans_11_20=1
			else:
				if rank>=21 and rank<=30:
					doc_21_30.append(doc)
					if aa==1:
						ans_21_30=1			
				else:
					if rank>=31 and rank<=40:
						doc_31_40.append(doc)
						if aa==1:
							ans_31_40=1
					else:
						if rank>=41 and rank<=50:
							doc_41_50.append(doc)
							if aa==1:
								ans_41_50=1
						else:
							if rank>=51 and rank<=100:
								doc_51_100.append(doc)
								if aa==1:
									ans_51_100=1
							else:
								if rank>=101 and rank<=200:
									doc_101_200.append(doc)
									if aa==1:
										ans_101_200=1
								else:
									if rank>=201 and rank<=300:
										doc_201_300.append(doc)
										if aa==1:
											ans_201_300=1
									else:
										if rank>=301 and rank<=400:
											doc_301_400.append(doc)
											if aa==1:
												ans_301_400=1
										else:
											if rank>=401 and rank<=500:
												doc_401_500.append(doc)
												if aa==1:
													ans_401_500=1										
			if doc==1:
				rank1.append(rank)
	...
	res=[]
	if len(doc_1_10)>0:
		res.append(float(sum(doc_1_10))/float(len(doc_1_10)))
	else:
		res.append(0.0)
	
	if len(doc_11_20)>0:
		res.append(float(sum(doc_11_20))/float(len(doc_11_20)))
	else:
		res.append(0.0)
		
	if len(doc_21_30)>0:
		res.append(float(sum(doc_21_30))/float(len(doc_21_30)))
	else:
		res.append(0.0)
		
	if len(doc_31_40)>0:
		res.append(float(sum(doc_31_40))/float(len(doc_31_40)))
	else:
		res.append(0.0)
		
	if len(doc_41_50)>0:
		res.append(float(sum(doc_41_50))/float(len(doc_41_50)))
	else:
		res.append(0.0)
		
	if len(doc_51_100)>0:
		res.append(float(sum(doc_51_100))/float(len(doc_51_100)))
	else:
		res.append(0.0)
		
	if len(doc_101_200)>0:
		res.append(float(sum(doc_101_200))/float(len(doc_101_200)))
	else:
		res.append(0.0)
	
	if len(doc_201_300)>0:
		res.append(float(sum(doc_201_300))/float(len(doc_201_300)))
	else:
		res.append(0.0)
		
	if len(doc_301_400)>0:
		res.append(float(sum(doc_301_400))/float(len(doc_301_400)))
	else:
		res.append(0.0)
	
	if len(doc_401_500)>0:
		res.append(float(sum(doc_401_500))/float(len(doc_401_500)))
	else:
		res.append(0.0)
	...
	if len(rank1)>0:
		res.append(float(sum(rank1))/float(len(rank1)))
	else:
		res.append(0.0)
	
	if len(rank2)>0:
		res.append(float(sum(rank2))/float(len(rank2)))
	else:
		res.append(0.0)
		
	if len(rank3)>0:
		res.append(float(sum(rank3))/float(len(rank3)))
	else:
		res.append(0.0)
	
	if len(rank4)>0:
		res.append(float(sum(rank4))/float(len(rank4)))
	else:
		res.append(0.0)
	
	if len(rank5)>0:
		res.append(float(sum(rank5))/float(len(rank5)))
	else:
		res.append(0.0)					
...
def call_main_GST(f1,f2,f4,f5,no_GST,gdict,verbose,gt1,config,h4,timech,qid,anspath):
    global threshold
    global MAX_MATCH
    ...
    if verbose:
        ...
        if verbose:
            ...
            sys.exit(2)	
    '''
    threshold=h4 #float(config['Cosine_threshold'])
    ...
    t2 = time.time() 
    #if timech>0 and time2-time1 > 2*timech:
    #print("This is taking some time returning None")
    #return 
    count={}
    for v in corner:
        for v1 in corner[v]:
    ...
    GST_set=[]
    GST_set=get_GST(Q,T,P,G,no_GST,corner,verbose,timech)
    ##To save in file
    with open(anspath+'_GSTs','wb') as f:
        pickle.dump(GST_set, f)

    #if len(GST_set)<=0:
    #with open(anspath,'a') as file: # Use file to refer to the file object
    #file.write(qid)
    #file.write("\n")
        ...
        for n in T.nodes():
            ...
            for gt in gt1:
                ...
                if len(nn)>0:
                    ...
                    if n==(gt.lower()):
                        aa_flag=1
        no_docs=get_doc_count(T,G)
        rank1=index+1
        rank_doc.append((rank1,no_docs,aa_flag))
    ...
    '''
    GST_set_doc=[]
    for tree_index in range(0,len(GST_set)):
        T1=GST_set[tree_index][1]
        #cost=get_cost(T1,G)
        doc_ct=get_doc_count(T1,G)
        match_flag1=0
        for n in T1.nodes():
            nn=n.split(':')	
            for gt in gt1:
	    ...
	    GST_set_doc.append((match_flag1,doc_ct))
	
    pickle.dump(GST_set_doc,open(f6,'w'))
    '''
    
    GST_set=set(GST_set); 
    #print("GST set = ",GST_set)
        ...
        for n in T.nodes():
            ...
            for gt in gt1:
                ...
                if len(nn)>0:
                    ...
                    if n==(gt.lower()):
                        match_flag=1
            if nn[1]=='Entity' and n not in corner:
                #Non entities and Cornernerstones are removed 
                type_chk=get_type_simi(n,G,ans_type,gdict)
                #print('Type check = ',type_chk)
                if verbose:
                    print ("\nType match ", n,type_chk)
                
                if type_chk>=type_threshold:
                    #(len(ans_type)>0 and type_chk>=0.5) or (len(ans_type)==0 and type_chk>=-1):#Answer type non matching are removed
                    tree_pot[(v,T)].add(n)	
                    #if verbose:
                        ...
                        if (n1,n2) in G.edges():
                            ...
                            if (n2,n1) in G.edges():
                                ...
                                if data['etype']=='Entity_Alignment':
                                    flag=1
                        if flag==1:
                            # and len(n2)>len(n1): #merging into the longer answer
                            if n1 not in match:
                                match[n1]=set()
                            match[n1].add(n2)	
                            if n2 not in match:
                            ...
                            if issubseq(n1,n2) or issubseq(n2,n1):
                                if n1 not in match:
                                    match[n1]=set()
                                match[n1].add(n2)	
                                if n2 not in match:
            ...
            if n in match:
                ...
                for n1 in match[n]:
                    if len(n1)>maxlen:
                        maxlen=len(n1)
                        maxw=n1
                tree_pot1[(v,T)].add(maxw)
            else:
                tree_pot1[(v,T)].add(n)
                														
    if verbose:
        print "\ntrees after merging --->",tree_pot1
    '''
    ...
    return len(G.nodes()),len(G.edges()), GST_set, match_flag, candidate_match_flag1,0, top_match_flag1, candidate_match_flag2,0, top_match_flag2, candidate_match_flag3,0, top_match_flag3, candidate_match_flag4, 0, top_match_flag4, candidate_match_flag5,0, top_match_flag5
